...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Write an article about ${input} in ${selectedArticleLength.text}.`;

      const { data } = await axios.post(
        "/api/ai/generate-article",
        { prompt, length: selectedArticleLength },
        {
    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      >
        ...
        <input
          ...
          placeholder="Masa depan kecerdasan buatan seperti apa..."
          required
        />
        ...
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {articleLength.map((item, index) => (
            <span
              onClick={() => setSelectedArticleLength(item)}
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${
                selectedArticleLength.text === item.text
